...
function InlineNewTranscriptionView({ onBack }: { onBack: () => void }) {
  ...
  const PLATFORMS = [
    ...
    { name: 'Twitter/X', color: isDark ? '#ffffff' : '#000000' },
    { name: 'Facebook',  color: '#1877F2' },
    { name: 'Twitch',    color: '#9146FF' },
  ];

  const STEPS = [
    { icon: <Link2 className="w-4 h-4" />, label: 'Paste links', desc: 'Drop up to 20 video URLs from any supported platform.' },
  ...
  const FEATURES = [
    { icon: <Clock className="w-3.5 h-3.5" />, label: 'Timestamps', desc: 'Word-level timing on every line.' },
    { icon: <Users className="w-3.5 h-3.5" />, label: 'Speaker detection', desc: 'Auto-labels multiple speakers.' },
    { icon: <Globe className="w-3.5 h-3.5" />, label: '50+ languages', desc: 'Transcribe & retranslate in one step.' },
    { icon: <Zap className="w-3.5 h-3.5" />, label: 'Fast turnaround', desc: 'Most videos ready in under 60 s.' },
  ];

  return (
    <div className="overflow-y-auto flex flex-col items-center justify-start px-6 pt-12 pb-16" style={{ background: pageBg }}>
      {/* Heading */}
      ...
      <div className="w-full max-w-2xl mb-0">
        <div className="border rounded-2xl transition-colors" style={{ background: isDark ? '#111111' : '#ffffff', borderColor: isFocused ? (isDark ? '#E3E3E3' : '#111111') : (isDark ? '#262626' : '#e5e7eb') }}>
          {/* Top row — textarea */}
          <div className="flex items-start gap-3 px-5 pt-4 pb-3">
            <textarea
              ...
              placeholder={"Paste up to 50 video links from TikTok, Instagram, YouTube, and more.\nWe'll extract, transcribe, and save them to your library."}
              rows={2}
      ...
      <div className="w-full max-w-2xl mt-10">
        <p className="text-xs uppercase tracking-widest mb-4 text-center" style={{ color: isDark ? 'rgba(255,255,255,0.28)' : '#9ca3af', fontWeight: 600 }}>
          Supported platforms
        </p>
        <div className="flex flex-wrap gap-2 justify-center">
          {PLATFORMS.filter(p => ['TikTok', 'YouTube', 'Instagram'].includes(p.name)).map(p => (
            <div
              key={p.name}
      ...
      {/* ── How it works ────────────────────────────────────────────────── */}
      <div className="w-full max-w-2xl mt-10">
        <p className="text-xs uppercase tracking-widest mb-4" style={{ color: isDark ? 'rgba(255,255,255,0.28)' : '#9ca3af', fontWeight: 600, textAlign: 'center' }}>
          How it works
        </p>
          ...
          {STEPS.map((step, i) => (
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <span
                  className="text-[10px] w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 ml-auto"
                  style={{ background: isDark ? 'rgba(255,255,255,0.04)' : '#f3f4f6', color: muted, fontWeight: 600 }}
                >
                  {i + 1}
                </span>
              </div>
              <div>
                <p className="text-xs mb-1" style={{ color: text, fontWeight: 600 }}>{step.label}</p>
                <p className="text-[11px]" style={{ color: muted, lineHeight: 1.6 }}>{step.desc}</p>
              </div>
            </div>
          ))}
        </div>
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden" style={{ background: bg }}>
      ...
      <AppSidebar
        ...
        onNewTranscript={() => { setShowNewTranscription(true); setSelectedTranscriptId(null); }}
      ...
      {showNewTranscription && (
        <div
          ...
          onClick={() => setShowNewTranscription(false)}
        >
          <div
            ...
            style={{
              ...
              border: `1px solid ${border}`,
              boxShadow: isDark ? '0 24px 80px rgba(0,0,0,0.7)' : '0 24px 80px rgba(0,0,0,0.18)',
            }}
          ...
          >
            ...
            <InlineNewTranscriptionView onBack={() => setShowNewTranscription(false)} />
          </div>
        </div>
      )}
